...
gam_type<-matrix(2,nrow=N_draws, ncol=length(spp_names)) #vector to store shape of gams: 0 horizontal, 1 linear, 2 non-linear
...
for (s in 1:length(spp_names)){
  ...
  x.mesh <- rep(seq(min(r_clim$SPEI),max(r_clim$SPEI),length=100)) 
   species.mesh<-rep(spp_names[s],each=100)
    ...
    if(length((df-2*df.sd)[(df-2*df.sd)<0] & (df+2*df.sd)[(df+2*df.sd)>0])>=95)
    {gam_type[i,s]<-0}
  ...
  ## Plot. Two options: 1) Plot everything 2) Only plot non-linear in blue and linear in red.
...
V_summary<-c()
for (s in 1:length(spp_names))
{V_summary<-rbind(V_summary,c(spp_names[s],summary(factor(gam_type[,s],levels=c(0,1,2)))))}
V_summary
```
...
# Show the effects of mean and variance across the non-horizontal gams
  for( s in 1:length(spp_names))
  { 
    pass<-mean_var_effect[s,gam_type[gam_type[,s]!=0,s],]
    colnames(pass)<-c("M","V","M*V")
    boxplot(pass,main=spp_names[s])
  }
